/*
Fully optimized matrix multiplication
*/
...
void matmul(float* A, float* B, float* C, int M, int N, int K) {
    ...
    for (int j = 0; j < N; j += NC) {
        int nc = min(NC, N - j);
        int kc = min(KC, K);
        pack_blockB(&B[j * K], blockB_packed, nc, kc, K); // all should be in L3 caching
        for (int i = 0; i < M; i += MC) {
            ...
            /*
            consider pack block from reverse
            Therefore, on the forward process the most recent memory is on L1 cache.
            Note that the caching on INTEL CORES follow a 
            */
            pack_blockA(&A[i], blockA_packed, mc, kc, M);
            
            /*
            makes sense to parallel here
            l3 cache is shared across cores. So, each core can get data from the l3 cache.
            
            however, you only need to populate the l3 cache first. Then, each core can focus on populating their own respective 
            L2 or L1 cores.
            */
            PRAGMA_OMP_PARALLEL_FOR
        ...
        /*
        test whether accumulating at the transpose poses any extra extra memory (shouldn't do that much...) 
        Therefore, we can operate matrix A and B assuming as contigious memory in of itself.
        */
        
        // most of this is store accumulation
        // Why where? why can't go to zero_init_accum func? better paral
        // _mm256 
